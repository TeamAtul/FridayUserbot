...
async def on_snip_delete(event):
    iloveindia = await edit_or_reply(event, "Yüklənir...")
    name = event.pattern_match.group(1)
    remove_filter(event.chat_id, name)
    await iloveindia.edit(f"filter {name} uğurla silindi.")


@borg.on(admin_cmd(pattern="rmfilters$"))
@borg.on(sudo_cmd(pattern="rmfilters$", allow_sudo=True))
async def on_all_snip_delete(event):
    await edit_or_reply(event, "Yüklənir....")
    remove_all_filters(event.chat_id)
    await sadness.edit(f"filters **Yalnız Bu Chatda** uğurla silindi")
