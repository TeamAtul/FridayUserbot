...
from fridayfridayfridayuserbot import CMD_HELP, LOGS, TEMP_DOWNLOAD_DIRECTORY
from fridayfridayfridayuserbot.events import register
...
async def download(target_file):
    """ For .dl command, download files to the fridayfridayfridayuserbot's server. """
    friday = await edit_or_reply(target_file, "`Processing ...`")
    await friday.edit("Processing using fridayfridayfridayuserbot server ( ◜‿◝ )♡")
...
async def upload(u_event):
    """ For .upload command, allows you to upload a file from the fridayfridayfridayuserbot's server """
    await u_event.edit("Processing ...")
    input_str = u_event.pattern_match.group(1)
    if input_str in ("fridayfridayfridayuserbot.session", "config.env"):
